...
// storedAllowedRootEntries selects the stored allowed-root entries from raw
// config: the legacy singular allowed_root and the rich allowed_roots array
// are the stored-config schema, and this owner resolves their migration
// selection (the ambiguous-schema, required, and non-empty rules) once for
// every allowed-root projection.
