...
// resolveAllowedRootsForRemove resolves stored roots for removal operations.
// It uses filesystem identity (not authorization policy) so that remove can
// repair an invalid config. For each stored root:
//   - requires absolute/non-empty syntax;
//   - tries filesystem canonicalization WITHOUT workspace policy;
//   - if path doesn't exist, uses cleaned absolute as stale identity;
//   - for unexpected errors: fails closed.
func resolveAllowedRootsForRemove(raw map[string]json.RawMessage, fc *fileConfig) ([]string, error) {
	hasLegacy := raw["allowed_root"] != nil
	hasNew := raw["allowed_roots"] != nil
	if hasLegacy && hasNew {
		return nil, fmt.Errorf("ambiguous configuration: both allowed_root and allowed_roots are present; migrate to allowed_roots and remove allowed_root")
	...
	for _, r := range roots {
		...
		// Try filesystem canonicalization without policy check.
		abs, err := filepath.Abs(r)
		if err != nil {
			return nil, fmt.Errorf("cannot resolve allowed_roots entry %q: %w", r, err)
		}
		canon, err := filepath.EvalSymlinks(abs)
		if err != nil {
			if os.IsNotExist(err) {
				// Path doesn't exist; use cleaned absolute as stale identity.
