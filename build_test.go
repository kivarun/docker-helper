...
func TestBuildContextSiblingDirectoryRejected(t *testing.T) {
	...
	sibling := filepath.Join(app.Config.AllowedRoots[0].Path, "sibling")
...
func TestBuildWorkspaceIsSymlink(t *testing.T) {
	...
	realDir := filepath.Join(app.Config.AllowedRoots[0].Path, "real-dir")
	if err := os.MkdirAll(realDir, 0755); err != nil {
		t.Fatalf("cannot create real dir: %v", err)
	}

	linkDir := filepath.Join(app.Config.AllowedRoots[0].Path, "link-dir")
