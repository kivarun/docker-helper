...
# Both entries come from the manual openSUSE UAT; the AppArmor policy is NOT
# widened to silence them — they are merely tolerated here.
is_allowlisted_deny() {
  local line="$1"
  ...
  if echo "$line" | grep -q 'operation="exec"' \
    && echo "$line" | grep -q 'name="/usr/libexec/git/git"' \
