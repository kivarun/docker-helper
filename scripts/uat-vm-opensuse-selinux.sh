...
# targeted regression groups 1-4 (scripts/uat-regressions-runner-selinux.sh).
...
# regressions 1-4) from executing; the final summary records every stage and
...
#       -> Release-2 SELinux targeted regression groups 1-4 (collect-all runner)
#
...
#                    mount-pin regression, socket micro-proof, regressions 1-4).
...
if [ "$MODE" = "ab-proof" ]; then
  ...
  # ---------------------------------------------------------------------------
  # 8c. Release-2 SELinux targeted regression groups 1-4 (collect-all)
  # ---------------------------------------------------------------------------
  log "== 8c. SELinux targeted regression groups 1-4 (collect-all runner) =="
  SELREG_RESULT=FAIL
  if run_guest_capture "SELinux regression groups 1-4 inside the guest" \
    ...
    log "SELinux regression groups 1-4 reported a failure (recorded)"
  fi
  record_stage "SELinux regressions (1-4)" "$SELREG_RESULT"
fi
