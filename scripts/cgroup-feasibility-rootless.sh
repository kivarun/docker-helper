...
as_user bash -c 'CTR=$(command -v containerd); echo "FACT: containerd-binary=$CTR"; mkdir -p $XDG_RUNTIME_DIR/docker/containerd $HOME/.local/share/docker/containerd/dir; nohup $CTR --root=$HOME/.local/share/docker/containerd/dir --state=$XDG_RUNTIME_DIR/docker/containerd/state --address=$XDG_RUNTIME_DIR/docker/containerd/containerd.sock --log-level=warn >> $HOME/containerd-feas.log 2>&1 < /dev/null & sleep 1' \
