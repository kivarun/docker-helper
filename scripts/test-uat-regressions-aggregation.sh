...
# selinux_stage_accept BB SELREG MP LIFECYCLE SELCHECK RUNDIR WLMAC MIG211 H6RP:
# every mandatory SELinux stage must be PASS (fail-closed) for the job to
# be eligible for success.
stage_accept() {
  ( SCRIPT_DIR="$SRC_DIR/scripts" \
      source "$SRC_DIR/scripts/uat-vm-opensuse-selinux-lib.sh" \
      && selinux_stage_accept "$1" "$2" "$3" "$4" "$5" "$6" "$7" "$8" "$9" )
}
stage_accept PASS PASS PASS PASS PASS PASS PASS PASS PASS && ok "VM: all nine stages PASS -> eligible for success" \
  || bad "VM: all nine stages PASS -> eligible for success"
if stage_accept PASS PASS PASS PASS PASS PASS PASS PASS FAIL; then
  bad "VM: H6RP_RESULT=FAIL must be a failure"
else
  ok "VM: H6RP_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS PASS PASS PASS PASS PASS PASS BLOCKED; then
  bad "VM: H6RP_RESULT=BLOCKED must be a failure"
else
  ok "VM: H6RP_RESULT=BLOCKED -> failure"
fi
if stage_accept PASS PASS PASS PASS PASS PASS PASS FAIL PASS; then
  bad "VM: MIG211_RESULT=FAIL must be a failure"
else
  ok "VM: MIG211_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS PASS PASS PASS PASS FAIL PASS PASS; then
  bad "VM: WLMAC_RESULT=FAIL must be a failure"
else
  ok "VM: WLMAC_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS PASS PASS PASS FAIL PASS PASS PASS; then
  bad "VM: RUNDIR_RESULT=FAIL must be a failure"
else
  ok "VM: RUNDIR_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS PASS PASS PASS PASS PASS BLOCKED PASS; then
  bad "VM: RUNDIR_RESULT=BLOCKED must be a failure"
else
  ok "VM: RUNDIR_RESULT=BLOCKED -> failure"
fi
if stage_accept PASS PASS PASS PASS FAIL PASS PASS PASS PASS; then
  bad "VM: SELCHECK_RESULT=FAIL must be a failure"
else
  ok "VM: SELCHECK_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS PASS BLOCKED PASS PASS PASS PASS PASS; then
  bad "VM: LIFECYCLE_RESULT=BLOCKED must be a failure"
else
  ok "VM: LIFECYCLE_RESULT=BLOCKED -> failure"
fi
if stage_accept PASS PASS PASS FAIL PASS PASS PASS PASS PASS; then
  bad "VM: LIFECYCLE_RESULT=FAIL must be a failure"
else
  ok "VM: LIFECYCLE_RESULT=FAIL -> failure"
fi
if stage_accept PASS PASS BLOCKED PASS PASS PASS PASS PASS PASS; then
  bad "VM: MP_RESULT=BLOCKED must be a failure"
else
  ok "VM: MP_RESULT=BLOCKED -> failure"
fi
if stage_accept PASS PASS FAIL PASS PASS PASS PASS PASS PASS; then
  bad "VM: MP_RESULT=FAIL must be a failure"
else
  ok "VM: MP_RESULT=FAIL -> failure"
fi
if stage_accept PASS FAIL PASS PASS PASS PASS PASS PASS PASS; then
  bad "VM: SELREG_RESULT=FAIL must be a failure"
else
  ok "VM: SELREG_RESULT=FAIL -> failure"
fi
if stage_accept FAIL PASS PASS PASS PASS PASS PASS PASS PASS; then
