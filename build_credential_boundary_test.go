...
// P3-D1/D1.1 credential-boundary proof through the production handlers and
...
// audit, public bodies). Builds are asynchronous operations, so every
// recorded invocation is associated with its ACTUAL operation ID (each
// driver child argv carries it) instead of assuming submission order.
// Synthetic children prove plumbing only; real buildctl registry
// authentication remains an integration/UAT item and no CA behavior is
// asserted here (deferred to D2).
...
// driverChildOpIDPattern is the canonical issued Operation ID shape
// (operationIDPrefix + operationIDHexLength hex characters) that every
// build-driver child carries in its argv: buildctl through the --addr
// socket path and the internal tag, load through the staged export path,
// inspect/tag/rmi through the internal tag.
var driverChildOpIDPattern = regexp.MustCompile(`op_[0-9a-f]{32}`)

// buildChildOpID associates a recorded build child invocation with the
// operation whose driver built it.
func buildChildOpID(t *testing.T, calls *recordedCalls, i int) string {
	t.Helper()
	id := driverChildOpIDPattern.FindString(strings.Join(calls.argv(i), "\x00"))
	if id == "" {
		t.Fatalf("child %d argv carries no operation ID:\n%v", i, calls.argv(i))
	}
	return id
}

// TestBuildDriverSessionDockerConfigIsolation proves the credential
// boundary with two Sessions carrying distinct credential markers: each
// build's buildctl child env is exactly its own Session's DOCKER_CONFIG
// directory (never the other Session's), no build child carries a
// --config flag, and neither marker appears in any observable surface —
// manager requests, child argv/env, operational logs, audit, or public
// response bodies (the sanitized failed-login public error included).
// Both sessions store distinct credentials before their builds; the
// failed login attempt (retried) proves the sanitized-error surface.
func TestBuildDriverSessionDockerConfigIsolation(t *testing.T) {
	...
	// --config directory exactly like real docker login would. Attempt
	// order: session A succeeds, session B's first attempt fails and
	// stores nothing, session B's retry succeeds and stores its marker.
	var loginAttempts int32
	app.ExecCommandContext = func(ctx context.Context, name string, args ...string) *exec.Cmd {
		if hasArgvWord(args, "login") {
			script := "exit 1"
			n := atomic.AddInt32(&loginAttempts, 1)
			if n == 1 || n == 3 {
				script = `printf '{"auths":{"registry.example.com":{"auth":"'}} > "$1/config.json"; cat >> "$1/config.json"; printf '"}}' >> "$1/config.json"`
			}
			// The fake login child keeps the handler's argv (its script
			// addresses the --config dir as $1) and reads the credential
			// from stdin, exactly like real docker login.
	...
	if wBRetried.Code != http.StatusOK {
		t.Fatalf("session B login retry: expected 200, got %d: %s", wBRetried.Code, wBRetried.Body.String())
	}

	// Stored credential state: each session's config dir contains ONLY
	// its own marker; the failed first attempt stored nothing that a
	// retry had to clean up.
	dataA, err := os.ReadFile(filepath.Join(dirA, "config.json"))
	if err != nil {
		t.Fatalf("session A credential not stored: %v", err)
	}
	if !strings.Contains(string(dataA), markerA) || strings.Contains(string(dataA), markerB) {
		t.Errorf("session A config must contain only its own marker")
	}
	dataB, err := os.ReadFile(filepath.Join(dirB, "config.json"))
	if err != nil {
		t.Fatalf("session B credential not stored by the retry: %v", err)
	}
	if !strings.Contains(string(dataB), markerB) || strings.Contains(string(dataB), markerA) {
		t.Errorf("session B config must contain only its own marker")
	}
	...
	// exactly its OWN Session's Docker config dir. The buildctl child is
	// associated with its actual operation ID, not submission order.
	for _, tc := range []struct {
		label string
		op    *operation
		dir   string
	}{
		{label: "session A", op: opA, dir: dirA},
		{label: "session B", op: opB, dir: dirB},
	} {
		idx := -1
		for i := 0; i < calls.count(); i++ {
			if strings.HasSuffix(calls.childName(i), "buildctl") && buildChildOpID(t, calls, i) == tc.op.ID {
				if idx >= 0 {
					t.Fatalf("multiple buildctl children for %s", tc.op.ID)
				}
				idx = i
			}
		}
		if idx < 0 {
			t.Fatalf("no buildctl child for %s:\n%s", tc.op.ID, calls.all())
		}
		env := calls.env(idx)
		if len(env) != 1 || env[0] != "DOCKER_CONFIG="+tc.dir {
			t.Errorf("%s buildctl env = %d entries (first %q), want exactly [DOCKER_CONFIG=%s]", tc.label, len(env), firstEnvEntry(env), tc.dir)
		}
	}
	...
	for _, marker := range []string{markerA, markerB} {
		...
		for _, surface := range []string{bodyA, bodyB, wA.Body.String(), wBFailed.Body.String(), wBRetried.Body.String()} {
