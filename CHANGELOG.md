...
- Issuance-time Session filesystem roots: the authority creating a Session (Admin, Principal credential, or Launcher credential) can issue, per created Session, additional absolute host filesystem roots through the new optional `filesystem_roots` field on `POST /sessions` and the repeatable `--filesystem-root PATH=ACCESS` flag on `session create` (PATH is an absolute host path inside the target Launcher's effective allowed roots; the workspace stays mandatory and receives the maximum permitted ceiling mode, and an explicit root at the workspace path narrows it). The request may only narrow the resolved target Launcher's effective ceiling: a `read_write` root under an effective `read_only` region, or a path outside the ceiling, is refused `400 invalid_filesystem_policy` before the Session exists. Omitted and empty `filesystem_roots` preserve the inherited behavior byte-for-byte; the issued request ends in the same immutable persisted multi-root snapshot, so no new persistence schema and no post-create Session filesystem mutation exist. Run mounts may address every issued root through the absolute spelling, authorized only through the issued snapshot. In user mode the capability stays workspace-only (user mode has no inode-pinned mounts): every requested `filesystem_roots` entry must be the canonical workspace, and the only accepted run-mount source is the workspace root itself. This completes the delegated orchestrator capability: a Launcher credential can issue each dynamically created run Session its own root set at issuance time and can never widen Launcher/Principal/global authority.
...
- Credential self-introspection: the new `GET /self` endpoint and `docker-helper self` command answer the authenticated credential's own resource document under one daemon-side classification. A Principal credential receives its Principal identity with stored and effective allowed-root entries, a Launcher credential its Launcher with stored (empty for inherit) and effective entries, and a Session bearer its Session's own `session show` body including the persisted immutable filesystem snapshot. The admin token receives the stable `404 self_not_available`; unknown, revoked, disabled, and expired credentials receive the non-disclosing 401 authentication contract; database failures are 500. The surface is read-only and grants no authority beyond the credential's existing scope, closing the Release 2.1 self-introspection debt.
- Issued-tree Session MAC preparation (architectural correction from the final SELinux UAT): Session MAC preparation covers every filesystem tree issued in the Session's immutable snapshot — the workspace and every additional issued root — through the same backend-neutral coordinator, so an issued read-write external root actually receives its SELinux `docker_helper_workspace_t` labeling (or AppArmor managed-boundary coverage) and is writable/readable as issued. Authorization ceilings (global/Principal/Launcher) remain MAC-free and never trigger relabeling merely by existing; nested issued transitions collapse to one boundary per tree; regular-file issued roots are supported as exact boundaries on both backends; a live operation's lease protects the Session's complete bound coverage set against a concurrent deletion; startup reconciliation rebuilds each live Session's binding from the persisted snapshot and retains every issued tree's coverage while workload state is still pending.
- CLI usability and legacy-surface cleanup (RC5): the CLI parser accepts interspersed flags — options may appear before, between, or after positional arguments until an explicit `--` (the retired "flags must precede positional arguments" refusal is gone; a bare `--` always terminates flags, and `--flag=--` still parses with the value `--`). Launcher allowed-root mutation grammar is PATH-first: `add/remove` take `PATH [LAUNCHER]` and `set-access` takes `PATH ACCESS [LAUNCHER]`. Completion follows the parser and the policy: flags are offered after positionals (used non-repeatable flags are suppressed, repeatable flags keep being offered), `launcher allowed-root add` completes PATH from the effective Principal ceiling with no host fallback, the existing-entity mutations complete exactly the stored roots they address (stored global roots from the local list, stored Principal roots via `completion roots principal --stored`, stored Launcher roots via the new `completion roots launcher`), and `principal allowed-root` mutations complete the USER positional from the daemon selector introspection. `/tmp` is now a wide namespace: the exact `/tmp` root stays forbidden as too broad, while `/tmp` descendants are accepted subject to the normal checks.
- Stored-root recovery inspection: `config allowed-root list` is a recovery-safe stored-config inspection — a stored root whose directory was deleted outside docker-helper keeps its cleaned canonical identity in the list, in the `--json` rich projection, and in the remove/set-access completion universe even when daemon startup or reload fail closed on it, so the operator can see and remove the stale entry to restore a startable config. Runtime validation is unchanged: startup, reload, and `add` still refuse a runtime config whose stored roots do not exist.
- Workload privilege floor (security closure SC1, findings C1/H9): every docker-helper run workload is now launched with the server-owned privilege floor — `--cap-drop ALL` and `--security-opt no-new-privileges:true`, emitted once by the run argv owner in every mode before any backend security option, with no request field that can disable or weaken it. The build staging owner strips `S_ISUID`/`S_ISGID` from every staged regular file at its single copy point (hardlinks included; sources untouched), so neither a hostile image nor workspace material can deliver a privilege-granting SUID/SGID executable to workload execution. The helper-socket runtime projection stays unchanged; with the floor in place the strongest reachable workload privilege is the server-owned execution identity, which the root-owned `0700` helper-private runtime state denies.
- Credential revocation race closure and unauthorized-path disclosure boundary (security closure SC1, findings H2/H3): a Principal or Launcher credential revoked or deleted between authentication and the Session-commit linearization point can no longer issue a new Session — the create re-proves its authorizing credential inside the commit transaction and answers with the same non-disclosing 401 credential contract as entry authentication, while already-issued Sessions stay valid and the admin path is unaffected. Session-create workspace admission no longer discloses host filesystem resolver detail for request spellings outside the effective allowed-root ceiling: every unauthorized filesystem state (existing, missing, dangling symlink, permission-denied) answers one bounded authorization-shape refusal with the diagnostic in the operational log, and the run/build data planes keep their stable non-disclosing public refusals with resolver diagnostics retained operationally. Authorized workspaces, symlink aliases, and symlink-escape protection are unchanged.
- Legacy 2.1 compatibility: path-only config entries, database rows, and API inputs are still accepted everywhere and mean `read_write`; `allowed_roots` is the one canonical field name on show/introspection surfaces (the canonical rich `{path, access}` representation; the legacy path-only string form remains the accepted config-file and scope-replacement input), and the Launcher complete-scope PUT keeps the documented path-only element form inside the same canonical field.
...
- When config.json is mutated by 2.2 (`config allowed-root add/set-access`, `reload`, `init`), string roots may be normalized into rich `{"path","access"}` objects on disk. The visible representations differ by contract: `config show` projects the canonical rich `allowed_roots` values; the default `allowed-root list` output is the path-only human compatibility form and `--json` prints the canonical rich array; `session show` separately displays the issued immutable filesystem snapshot with each entry's access mode (it is not a global allowed-root dual projection). Reading a 2.1.1 config unchanged stays valid.
