...
- When config.json is mutated by 2.2 (`config allowed-root add/set-access`, `reload`, `init`), string roots may be normalized into rich `{"path","access"}` objects on disk. The visible representations differ by contract: `config show` projects both the authoritative rich `allowed_root_entries` and the 2.x path-only `allowed_roots` compatibility form; the default `allowed-root list` output is the path-only human compatibility form and `--json` prints the canonical rich entry array; `session show` separately displays the issued immutable filesystem snapshot with each entry's access mode (it is not a global allowed-root dual projection). Reading a 2.1.1 config unchanged stays valid.
