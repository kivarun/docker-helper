...
- Issuance-time Session filesystem narrowing: the authority creating a Session (Admin, Principal credential, or Launcher credential) can request, per created Session, which parts of its workspace are `read_write` and which are `read_only` through the new optional `filesystem_entries` field on `POST /sessions` and the repeatable `--filesystem-entry PATH=ACCESS` flag on `session create` (PATH is workspace-relative, `.` for the workspace root is required). The request may only narrow the resolved target Launcher's effective ceiling: a `read_write` entry under an effective `read_only` region, or a path outside the ceiling, is refused `400 invalid_filesystem_policy` before the Session exists. Omitted `filesystem_entries` preserves the inherited behavior byte-for-byte; the narrowed request ends in the same immutable persisted snapshot, so no new persistence schema and no post-create Session filesystem mutation exist. This completes the dynamic pipeline-run capability: a Launcher credential can narrow each dynamically created run Session at issuance time and can never widen Launcher/Principal/global authority.
