...
- Allowed-root recovery: `config allowed-root list` is the recovery-safe stored-config inspection — a stored root whose directory was deleted outside docker-helper is still listed at its cleaned absolute identity (with `--json` carrying the stored access mode), so the stale entry that fails daemon startup stays discoverable while runtime validation stays fail closed. The PATH completion of `config allowed-root remove` and `config allowed-root set-access` offers this stored-root universe instead of generic filesystem completion, so the stale entry is completable and the removal restores daemon startup; a failed stored-root query degrades silently. Runtime config validation is unchanged: a missing stored root still makes daemon startup and reload fail closed. New targeted UAT regression group 19 (`scripts/uat-regression-allowed-root-recovery.sh`) covers the whole recovery scenario.
