...
- Legacy 2.1 compatibility: path-only config entries, database rows, and API inputs are still accepted everywhere and mean `read_write`; `allowed_root_entries` is the only output projection on show/introspection surfaces (the 2.x path-only `allowed_roots` output projection is retired — the same name remains the accepted config-file and scope-replacement input), and the Launcher complete-scope PUT keeps the documented path-only form alongside the canonical rich form.
