...
func TestReloadNoCASideEffectCLI(t *testing.T) {
	...
	// Remove openssl from PATH so CA preparation would fail if attempted.
	emptyBin := t.TempDir()
	t.Setenv("PATH", emptyBin)

	// Start a fake daemon that accepts reload.
	listener, srv := setupFakeReloadServer(t, socketPath)
	defer func() {
		srv.Close()
		listener.Close()
	...
	// Verify no trusted-ca artifacts were created on the client side.
...
func TestSessionListNoCASideEffectCLI(t *testing.T) {
	...
	// Remove openssl from PATH.
	emptyBin := t.TempDir()
	t.Setenv("PATH", emptyBin)

	// Start a fake daemon that returns an empty session list.
	listener, srv := setupFakeSessionListServer(t, socketPath)
	defer func() {
		srv.Close()
		listener.Close()
		os.Remove(socketPath)
	...
	// Verify no trusted-ca artifacts were created.
	trustedCADir := filepath.Join(runtimeDir, "docker-helper", "trusted-ca")
	if _, err := os.Stat(trustedCADir); !os.IsNotExist(err) {
...
func TestSessionCleanupNoCASideEffectCLI(t *testing.T) {
	...
	// Derive state dir from XDG_STATE_HOME (same env var that session cleanup uses).
	stateHome := os.Getenv("XDG_STATE_HOME")
	dbPath := filepath.Join(stateHome, "docker-helper", "docker-helper.db")

	// Initialize the database so cleanup can operate on it.
	...
	// Verify socket exists (from setupReloadTestEnv).
	_ = socketPath
	...
	// session cleanup operates directly on the database, no daemon needed.
...
// setupFakeReloadServer creates a Unix socket server that accepts a reload POST.
func setupFakeReloadServer(t *testing.T, socketPath string) (net.Listener, *http.Server) {
	t.Helper()
	os.Remove(socketPath)
...
// setupFakeSessionListServer creates a Unix socket server that returns an empty session list.
func setupFakeSessionListServer(t *testing.T, socketPath string) (net.Listener, *http.Server) {
	t.Helper()
	os.Remove(socketPath)
