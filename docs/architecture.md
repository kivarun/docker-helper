...
| `invalid_filesystem_policy` | `POST /sessions` | the supplied `filesystem_roots` is malformed or is not a narrowing of the effective Launcher ceiling (issuance-time refusal; no Session exists). The message is the bounded non-disclosing policy refusal (`invalid session filesystem policy`); a requested root that was admitted by the lexical ceiling proof but whose privileged resolution or stat failed answers with the path-resolution message `requested filesystem root does not exist or cannot be resolved` — still without any errno or resolved pathname (the internal diagnostic stays in the operational log) — so a nonexistent/unresolvable requested root is distinguishable from a genuine ceiling/access-mode refusal |
