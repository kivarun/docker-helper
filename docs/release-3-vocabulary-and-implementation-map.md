...
7. Docker CLI arguments, container IDs, process IDs, and attach framing are backend details unless a design explicitly promotes them into the public contract.
...
| `operationSupervisor` | In-memory registry, shutdown gate, pruning, cancel, and process termination. | No single equivalent. | Split into persistent store, durable worker, and live-process supervision. Retire the old name after callers migrate. | D0 |
...
| `terminateForShutdown` and `terminateOperations` | Graceful and forced process shutdown. | Live process supervision during daemon shutdown. | Preserve the proven bounded termination mechanics in a process-runtime component, separate from the durable Operation record. | D0 |
...
Container backend access should be isolated behind one domain adapter rather than added as more unrelated `newDockerCommand` calls across handlers. Whether that adapter continues using the Docker CLI or adopts a Docker Engine client is a separate technology decision; the public domain contract must not depend on that choice.
...
| `operation_log_max_bytes` | Bounds pull output and build/run Operation buffers. | Rename or replace as a bounded direct-command output limit. It must not remain described as Operation-log retention. |
| `audit_enabled` | Enables structured audit stream. | Retain. It is not the workload-output logging switch. |
| — | Optional workload-output emission to structured operational logging. | No corresponding option exists at the baseline commit. The operator option, scope, and redaction rules must be designed before documentation can name it. |
...
### 2. Workload-output logging option

The current configuration has no dedicated option corresponding to the Release 3 statement that workload output is logged only when enabled. `audit_enabled` is not a substitute.

The option name, affected Commands, redaction, size behavior, and reload semantics remain open.

### 3. ManagedContainerID prefix
