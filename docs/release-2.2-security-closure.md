...
| **H1** | Builder can fetch arbitrary URLs from a network position unavailable to the agent | **ACCEPTED_CONTRACT** | SC3 → Release 2.4 | Release-owner decision (2026-09-16, architectural): the builder network position is one visible consequence of the broader build execution boundary — an untrusted Dockerfile's `RUN` instructions execute as builder-root inside the current rootful Docker/BuildKit builder environment, a materially weaker boundary than the `run` contract (Principal UID:GID plus the server-owned privilege floor), and the two are not equivalent. The residual boundary is accepted for Release 2.2: this release does not promise a build sandbox or build network isolation, and build/registry surfaces keep their existing contracts unchanged (no production behavior or public API change). A local network-only fix is not considered sufficient: `--network none`, Dockerfile inspection/filtering, or similar partial measures would close one symptom of the boundary while the weaker builder execution boundary itself stays unredesigned, and Dockerfile filtering is the explicitly forbidden substitute security parser (`release-2.4-build-sandbox.md`, "Non-solution"). The complete elimination of this risk class belongs to the already accepted Release 2.4 Build Sandbox direction: builder-root separated from host-root authority, and the builder's network position made an explicitly server-owned part of the sandbox contract (never silently escalated; no caller-controlled builder entitlements; no hidden partial closure). |
...
| **H10** | An allowed root lets the root daemon read files the Principal could not read under Unix DAC | **ACCEPTED_CONTRACT** | SC3 | Release-owner decision (2026-09-16): `allowed_root` and the issued Session filesystem snapshot are an explicitly granted filesystem capability, NOT a path ceiling layered over the Principal's Unix DAC. In system mode the root-owned helper may perform the necessary helper-mediated reads inside the granted capability regardless of whether the specific Principal could read the same inode through its own host Unix credentials. Consequences recorded explicitly: build staging is the one current helper content-ingest path — a file inside the workspace capability may enter the staged build context (and the builder's view) even when the host Principal could not read it under DAC, as deliberate capability semantics, not a missed check; `read_only` is an access/integrity mode inside the granted capability and is never a confidentiality promise against the helper; actual workload file access is additionally evaluated by kernel DAC, including POSIX ACLs, against the credentials actually supplied to the container (current workload identity: Principal UID:GID with no capability bypass) plus the privilege floor, not a reproduction of the Principal's host login credential set: host supplementary groups are not propagated, so permissions depending on those group memberships may differ; user mode has no separate H10 gap because the non-root daemon is naturally bounded by its own DAC (an implementation consequence, not a different capability model); Release 2.4 does not automatically "close H10" — the build sandbox redesigns the builder execution/root/network boundary, and moving staging/read identity under an unprivileged Principal identity would be a separate explicit contract change, never a silent side effect. Production code is unchanged; owner-UID checks, mode-bit emulation, ACL parsers and "check-as-user" subsystems are explicitly not added. |
| **M1** | Environment/build secret values appear in the Docker CLI process argv | **ACCEPTED_CONTRACT** | SC3 → Release 3 | Release-owner decision (2026-09-16, Option 1a): the residual is the **daemon-side legacy Docker CLI argv** and it is accepted for Release 2.2. Inventory and per-channel disposition: (1) `run` environment values are passed to the daemon-side `docker` child as `--env NAME=VALUE`, so a value is present in that child's argv for the child's whole execution — a local process may observe it through `/proc/<pid>/cmdline` where the host procfs policy permits such observation; this is a deliberately accepted Release 2.2 residual, already documented in the shipped docs. (2) build-arg values are passed as `--build-arg K=V` with the same daemon-side argv residual; build args are explicitly NOT a secret transport and must not be used for secrets, and Docker/BuildKit may additionally retain ARG-related material in image history/provenance — a property of build semantics that does not disappear with a future argv removal. (3) registry login carries no residual: the password is delivered via `--password-stdin` and never appears in argv, environment, logs, or audit. No `--env-file` transport, dual transport, temporary secret-file subsystem, env-grammar narrowing, new API, or BuildKit secret knobs are introduced, and no per-variable "looks like a secret" filtering is attempted: a partial closure of `run` only (and only of the subset of the arbitrary-string env contract a file grammar can represent) would leave `build_args` exposed and create the false impression that the CLI transport became secret-safe. The argv class closes with the accepted Release 3 migration of build/run from the Docker CLI to a docker-helper-owned Docker Engine API adapter; that migration removes the CLI argv exposure but does NOT make build args a secret mechanism (build args remain non-secret input unless a separate future BuildKit secret contract is explicitly accepted). |
...
shim is denied by the shipped policy and never blocks; the shim's shebang is
an interpreter the shipped policy grants the daemon for that frontend —
`/bin/sh` on the AppArmor guest, `python3` (semanage's own interpreter) on
the SELinux guest, whose bash execution is denied; on the SELinux guest the
shim also carries the original binary's type via `chcon` so the confined
...
Queue-closure correction (review round on the single-transition fix): the
whole-transition budget bounds ONE in-flight transition, but the create's
lifecycle boundary was still entered with a blocking mutex acquisition and
the budget is created only later, inside `CreateSessionBinding` — so
Session creates arriving while the coordination is held queued on the
mutex and each one, once served, obtained its own fresh whole-transition
budget. An emergency disable queued behind N creates was therefore
delayed by about (N+1) budgets with no fixed bound independent of the
queued create count, contradicting the H8 closure claim. RED evidence
(deterministic, both backends, no sleeps-as-proof): with one parked
create holding the coordination, four further creates issued behind it
and the disable requested after them, the disable did not settle within
one in-flight transition budget plus slop (measured pre-fix with a 2 s
test budget: the disable completed only after 5 budget intervals — one
per queued create plus the in-flight one — the delay grows linearly with
the queued count). The fix keeps the existing owners: the create's
lifecycle boundary is admitted non-waiting (`TryLock` in
`createSessionAuthorized`), refusing a create that arrives while the
coordination is held with the typed `ErrLifecycleBusy` refusal before any
policy resolution or MAC work — nothing is queued, nothing resolves
policy state against a ceiling that may change, nothing commits, and
administrative lifecycle operations keep their blocking acquisition.
GREEN evidence (both backends, `go test -race` clean): every queued
create receives the stable refusal and the disable commits within one
in-flight budget plus slop; the in-flight create keeps its
whole-transition budget (`ErrMACTransitionBudgetExceeded` inside the
`ErrMACPreparation` chain); no Session exists anywhere; operation
admission is closed after the disable; and the next ordinary MAC
transition succeeds after the hostile condition is removed.
`TestH8CreateLifecycleBusyHTTPClass` proves the public boundary: the real
route handler answers `503 lifecycle_busy`, the audit record carries the
same class, and no Session is committed. The two mirror linearization
race tests adapt without weakening their invariants: a create concurrent
with a held boundary is now refused before any ceiling read (provably
...
**Queue:** empty. All three SC3 findings carry accepted release-owner
dispositions (H1, H10, M1 — all `ACCEPTED_CONTRACT`, 2026-09-16, see the
disposition matrix). SC3 completes with the review/merge of the
disposition PR: the change set is security dispositions plus
documentation/invariant tests only — production behavior and the public
API are unchanged, so no new hostile exact-candidate UAT is required.
No `--env-file` transport, dual transport, temporary secret-file
subsystem, or env-grammar narrowing is introduced (M1 decision rationale
in the matrix).

- **H1 builder network:** RESOLVED (2026-09-16, release owner) — the current
  build execution/network boundary is accepted for Release 2.2 as
  `ACCEPTED_CONTRACT`: the builder network position is one consequence of the
  broader build execution boundary (an untrusted Dockerfile's `RUN` executes
  as builder-root in the current rootful Docker/BuildKit builder
  environment — not the `run` contract), Release 2.2 promises no build
  sandbox and no build network isolation, and no partial closure
  (`--network none`, Dockerfile filtering) is attempted. The complete
  elimination of the risk class is owned by the accepted Release 2.4 Build
  Sandbox design ([`release-2.4-build-sandbox.md`](release-2.4-build-sandbox.md)):
  builder-root separated from host-root authority and the builder network
  position as an explicitly server-owned part of the sandbox contract.
  Production behavior and the public build API are unchanged in 2.2.
- **H10 DAC inheritance:** RESOLVED (2026-09-16, release owner) — accepted as
  `ACCEPTED_CONTRACT`: the granted filesystem capability (allowed root /
  issued Session filesystem snapshot) is the read authority for the
  helper-mediated access inside it; the root-owned system-mode helper may
  read inside the capability regardless of the Principal's own host Unix
  DAC reach. No owner-UID check, no mode-bit emulation, no ACL parser and
  no check-as-user subsystem are added; production code is unchanged. The
  workload-facing reads stay evaluated by kernel DAC, including POSIX
  ACLs, against the credentials actually supplied to the container
  (Principal UID:GID, privilege floor); this is not a reproduction of the
  Principal's host login credential set — host supplementary groups are
...
- **M1 secret transport:** RESOLVED (2026-09-16, release owner, Option 1a) —
  ...
  `/proc/<pid>/cmdline` while the child runs, where host procfs policy
  permits) and build-arg values as `--build-arg K=V` with the same residual;
  build args are explicitly not a secret transport (BuildKit may also retain
  ARG-related material in image history/provenance), and registry login
  carries no residual (`--password-stdin`). The argv class closes with the
  accepted Release 3 build/run Engine API adapter migration, which removes
  the CLI argv exposure without making build args a secret mechanism.
