...
#                   completion, pinned BuildKit payload) without rebuilding
#                   anything. The canonical producer (scripts/release-candidate.sh)
#                   builds the payload exactly once through the canonical
#                   builders and passes it to every artifact builder, so
#                   tar/DEB/RPM all pack the same bytes.
#
#   (default)       developer path: build the payload first through the
#                   canonical builders (build-static.sh, build-selinux-policy.sh,
#                   build-manpages.sh, build-buildkit-payload.sh) and generate
#                   the Bash completion from the built binary.
#
# Requirements:
...
# BuildKit payload members staged by the single pinned payload owner
# (build-buildkit-payload.sh); nFPM packs them into /usr/libexec/
# docker-helper/buildkit and /usr/share/doc/docker-helper/buildkit.
BUILDKIT_PAYLOAD_MEMBERS=(
...
if [[ -n "$PAYLOAD_DIR" ]]; then
	...
	# build-manpages.sh, build-buildkit-payload.sh) remain the single owners
	# of compilation and payload staging.
	...
	for member in docker-helper docker_helper.pp \
		...
		completions/docker-helper "${BUILDKIT_PAYLOAD_MEMBERS[@]}"; do
	...
	rm -rf "${SCRIPT_DIR}/dist/buildkit"
	cp -r "$PAYLOAD_DIR/buildkit" "${SCRIPT_DIR}/dist/buildkit"
else
	...
	# Stage the pinned BuildKit payload through its single owner.
	"${SCRIPT_DIR}/build-buildkit-payload.sh" "${SCRIPT_DIR}/dist/buildkit"
fi
...
# Verify the pinned BuildKit payload was staged (fail closed before packing).
for member in "${BUILDKIT_PAYLOAD_MEMBERS[@]}"; do
	if [[ ! -s "${SCRIPT_DIR}/dist/$member" ]]; then
		echo "error: staged BuildKit payload member missing or empty: dist/$member" >&2
		exit 1
	fi
done
