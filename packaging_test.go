...
func TestSkillAgentContract(t *testing.T) {
	...
	// the operator `session show` lookup must be documented with the
	// canonical positional identity grammar (`session show SESSION_ID`,
	// like the other resource show commands) and must not be presented as
	// the Session-bearer surface.
	if !strings.Contains(content, "session show SESSION_ID") {
		t.Error("SKILL.md must document the canonical `session show SESSION_ID` operator lookup syntax")
	}
	if strings.Contains(content, "session show --id") {
		t.Error("SKILL.md must not teach the retired `session show --id` syntax")
	}
