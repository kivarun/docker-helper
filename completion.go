...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "        if _docker_helper_complete_policy_roots \"$mode\" \"$prefix\" \"$flag\"; then")
	fmt.Fprintln(w, "            return")
	fmt.Fprintln(w, "        fi")
	fmt.Fprintln(w, "    fi")
	...
	fmt.Fprintln(w, "# line (--endpoint, --token-file) so the completion query")
	...
	fmt.Fprintln(w, "# --launcher ki. A query failure or an empty answer is a silent")
	fmt.Fprintln(w, "# degradation: the suggestions stay empty and the completion")
	fmt.Fprintln(w, "# process succeeds (exit status 0), never a crashed completion.")
	...
	fmt.Fprintln(w, "        return 0")
	fmt.Fprintln(w, "    fi")
	fmt.Fprintln(w, "    [ -n \"$vals\" ] || return 0")
	...
	fmt.Fprintln(w, "    _docker_helper_complete_within_roots \"$kind\" \"$roots\" \"$prefix\"")
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	...
	fmt.Fprintln(w, "# — never the generic host filesystem. An unavailable or empty list")
	fmt.Fprintln(w, "# is a silent degradation: the suggestions stay empty and the")
	fmt.Fprintln(w, "# completion process succeeds.")
	...
	fmt.Fprintln(w, "# the operator overrides are forwarded like every other query. An")
	...
	fmt.Fprintln(w, "    local prefix=\"$1\"")
	fmt.Fprintln(w, "    # This function owns COMPREPLY: the union prefix already saved")
	fmt.Fprintln(w, "    # sel_reply, so a failed or empty roots query must present an")
	fmt.Fprintln(w, "    # empty reply, never the selector candidates left over in")
	fmt.Fprintln(w, "    # COMPREPLY (which would duplicate them in the union).")
	fmt.Fprintln(w, "    COMPREPLY=()")
	fmt.Fprintln(w, "    local -a opargs=() rootsargs=()")
	fmt.Fprintln(w, "    mapfile -d '' -t opargs < <(_docker_helper_operator_args \"$cmd_path\")")
	fmt.Fprintln(w, "    local p")
