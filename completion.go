...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "    # the CLI parser, so a PATH containing '=' completes exactly as it")
	fmt.Fprintln(w, "    # parses. The PATH side (no '=' yet) falls through to the daemon-")
	fmt.Fprintln(w, "    # backed policy-roots machinery below.")
	fmt.Fprintln(w, "    case \"$flag\" in")
	...
	fmt.Fprintln(w, "    # already exactly typed.")
