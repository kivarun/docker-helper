...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "            return")
	fmt.Fprintln(w, "            ;;")
	fmt.Fprintln(w, "        \"config allowed-root\")")
	fmt.Fprintln(w, "            # Determine if we need ACTION or PATH")
	fmt.Fprintln(w, "            local pos_count=0")
	fmt.Fprintln(w, "            local action_word=\"\"")
	fmt.Fprintln(w, "            local j=1")
	fmt.Fprintln(w, "            while [ $j -lt $COMP_CWORD ]; do")
	fmt.Fprintln(w, "                local w=\"${COMP_WORDS[$j]}\"")
	fmt.Fprintln(w, "                case \"$w\" in")
	fmt.Fprintln(w, "                    docker-helper|config|allowed-root) ;;")
	fmt.Fprintln(w, "                    -*) ;;")
	fmt.Fprintln(w, "                    *)")
	fmt.Fprintln(w, "                        if [ $pos_count -eq 0 ]; then")
	fmt.Fprintln(w, "                            action_word=\"$w\"")
	fmt.Fprintln(w, "                        fi")
	fmt.Fprintln(w, "                        pos_count=$((pos_count + 1))")
	fmt.Fprintln(w, "                        ;;")
	fmt.Fprintln(w, "                esac")
	...
	fmt.Fprintln(w, "            done")
	fmt.Fprintln(w, "            if [ $pos_count -eq 0 ]; then")
	fmt.Fprintln(w, "                # Complete ACTION")
	fmt.Fprintln(w, "                COMPREPLY=( $(compgen -W \"list add remove\" -- \"$cur\") )")
	fmt.Fprintln(w, "            elif [ $pos_count -eq 1 ]; then")
	fmt.Fprintln(w, "                # Complete PATH based on action")
	fmt.Fprintln(w, "                case \"$action_word\" in")
	fmt.Fprintln(w, "                    add)")
	...
	fmt.Fprintln(w, "                    ;;")
	fmt.Fprintln(w, "                    *)")
	fmt.Fprintln(w, "                    # Unknown action or partial: no filesystem completion")
	fmt.Fprintln(w, "                    ;;")
	fmt.Fprintln(w, "                esac")
	fmt.Fprintln(w, "            fi")
	fmt.Fprintln(w, "            return")
