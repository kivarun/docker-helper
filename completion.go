...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "# COMP_POINT by the shell's own argument rules — unquoted whitespace")
	fmt.Fprintln(w, "# separates arguments, backslash escapes the next character, single")
	fmt.Fprintln(w, "# quotes are literal, double quotes honor backslash escapes — so")
