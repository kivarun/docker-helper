...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "                return")
	fmt.Fprintln(w, "            fi")
	fmt.Fprintln(w, "            # Stored-root universe: the existing-entity identities")
	fmt.Fprintln(w, "            # this targeted removal matches against. A stored root")
	fmt.Fprintln(w, "            # whose directory was deleted stays offered, so the stale")
	fmt.Fprintln(w, "            # entry that fails daemon startup stays recoverable.")
	...
	fmt.Fprintln(w, "            # pos 0 is the PATH, completed from the same stored-root")
	fmt.Fprintln(w, "            # universe as remove (the existing-entity identities this")
	fmt.Fprintln(w, "            # targeted mutation matches against); pos 1 is the static")
	...
	fmt.Fprintln(w, "}")
	fmt.Fprintln(w)
	fmt.Fprintln(w, "# Complete the PATH positional of `config allowed-root remove`/`set-access`")
	fmt.Fprintln(w, "# from the stored global allowed-root identities: the same existing-entity")
	fmt.Fprintln(w, "# universe both targeted mutations match against (config allowed-root")
	fmt.Fprintln(w, "# list, the recovery-safe stored-config inspection), so a stored root")
	fmt.Fprintln(w, "# whose directory was deleted outside docker-helper stays offered and")
	fmt.Fprintln(w, "# the stale entry that fails daemon startup stays completable for")
	fmt.Fprintln(w, "# recovery. This is never generic filesystem completion: every candidate")
	fmt.Fprintln(w, "# is a stored identity, and nothing that is not stored is addressable by")
	fmt.Fprintln(w, "# these mutations. The query is a local stored-config inspection; a")
	fmt.Fprintln(w, "# failure degrades silently to no suggestions instead of stalling the")
	fmt.Fprintln(w, "# shell.")
	fmt.Fprintln(w, "_docker_helper_complete_config_stored_roots() {")
	fmt.Fprintln(w, "    local prefix=\"$1\"")
	fmt.Fprintln(w, "    local roots r")
	fmt.Fprintln(w, "    if ! roots=\"$(${_docker_helper_WORDS[0]} config allowed-root list 2>/dev/null)\"; then")
	fmt.Fprintln(w, "        return")
	fmt.Fprintln(w, "    fi")
	fmt.Fprintln(w, "    compopt -o filenames 2>/dev/null || true")
	fmt.Fprintln(w, "    local -a comp=()")
	fmt.Fprintln(w, "    while IFS= read -r r; do")
	fmt.Fprintln(w, "        [ -n \"$r\" ] || continue")
	fmt.Fprintln(w, "        case \"$r\" in")
	fmt.Fprintln(w, "            \"$prefix\"*) comp+=(\"$r\") ;;")
	fmt.Fprintln(w, "        esac")
	fmt.Fprintln(w, "    done <<< \"$roots\"")
	fmt.Fprintln(w, "    COMPREPLY=(\"${comp[@]}\")")
	fmt.Fprintln(w, "}")
