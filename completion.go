...
func generateBashCompletion(w io.Writer) {
	...
	fmt.Fprintln(w, "    # shared by every --access flag and by the ACCESS side of the")
	fmt.Fprintln(w, "    # repeatable session-create --filesystem-entry PATH=ACCESS flag.")
	fmt.Fprintln(w, "    case \"$flag\" in")
	fmt.Fprintln(w, "        access)")
	fmt.Fprintf(w, "            COMPREPLY=( $(compgen -W %q -- \"$prefix\") )\n", strings.Join(allowedRootAccessVocabulary(), " "))
	fmt.Fprintln(w, "            ;;")
	fmt.Fprintln(w, "        filesystem-entry)")
	fmt.Fprintln(w, "            if [[ \"$prefix\" == *=* ]]; then")
	fmt.Fprintln(w, "                # Same grammar as the CLI parser: the split is on the")
	fmt.Fprintln(w, "                # LAST '=' (\"${prefix%=*}\"), so a PATH containing '='")
	fmt.Fprintln(w, "                # completes exactly as it parses.")
	fmt.Fprintln(w, "                local _fse_head=\"${prefix%=*}=\"")
	fmt.Fprintln(w, "                local _fse_acc=\"${prefix##*=}\"")
	fmt.Fprintln(w, "                local _fse_i")
	fmt.Fprintf(w, "                COMPREPLY=( $(compgen -W %q -- \"$_fse_acc\") )\n", strings.Join(allowedRootAccessVocabulary(), " "))
	fmt.Fprintln(w, "                for _fse_i in \"${!COMPREPLY[@]}\"; do")
	fmt.Fprintln(w, "                    COMPREPLY[$_fse_i]=\"$_fse_head${COMPREPLY[$_fse_i]}\"")
	fmt.Fprintln(w, "                done")
	fmt.Fprintln(w, "            fi")
	fmt.Fprintln(w, "            ;;")
	fmt.Fprintln(w, "    esac")
