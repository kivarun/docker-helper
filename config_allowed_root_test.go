...
// TestResolveStoredAllowedRootsRecoveryUniverse proves the stored-config
// inspection projection (the recovery-safe universe of `config allowed-root
// list` and the existing-entity completion of remove/set-access): each stored
// entry is projected to its canonical identity through the shared
// resolveAllowedRootIdentity owner, so a missing stored root stays visible
// and addressable (ENOENT keeps the cleaned absolute identity) while
// non-ENOENT resolution failures still fail closed.
