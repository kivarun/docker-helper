...
func newTestManager(active func() (bool, bool, error)) *selinuxFcontextManager {
	return &selinuxFcontextManager{
		...
		treeKind:      func(string) (selinuxTreeKind, error) { return selinuxTreeDirectory, nil },
	}
}
...
func TestSELinuxPolicySemanageTransition(t *testing.T) {
	...
	content := string(data)
	// The Session MAC preparation (ensureTreeFcontext) lists and adds
...
func TestSELinuxPolicySELinuxWorkspaceLock(t *testing.T) {
	...
	content := string(data)
	// ensureTreeFcontext acquires /run/lock/docker-helper-selinux.lock
...
func TestEnsureWorkspaceFcontextConflictingRule(t *testing.T) {
	...
	_, err := mgr.ensureTreeFcontext("/data", selinuxTreeDirectory)
	if err == nil {
	...
	mgr.runCommand = func(cmd string, args ...string) ([]byte, error) {
		if len(args) > 0 && args[0] == "fcontext" {
			if args[1] == "-l" {
				...
				// rule, the rollback removal must discover it by stem.
				if ruleAdded {
					return []byte("/data(/.*)?  gen_context(system_u:object_r:docker_helper_workspace_t:s0)"), nil
				}
				return []byte{}, nil
...
func TestOverlapAncestorLocalRule(t *testing.T) {
	...
	_, err := mgr.ensureTreeFcontext("/projects/data", selinuxTreeDirectory)
	if err == nil {
...
func TestOverlapEscapedPathNested(t *testing.T) {
	...
	_, err := mgr.ensureTreeFcontext("/data.test", selinuxTreeDirectory)
	if err == nil {
...
func TestOverlapEscapedPathSibling(t *testing.T) {
	...
	created, err := mgr.ensureTreeFcontext("/data.test", selinuxTreeDirectory)
	if err != nil {
...
func TestOverlapEscapedPathBracket(t *testing.T) {
	...
	_, err := mgr.ensureTreeFcontext("/project[1]", selinuxTreeDirectory)
	if err == nil {
...
func TestOverlapEscapedPathPlusSibling(t *testing.T) {
	...
	created, err := mgr.ensureTreeFcontext("/foo+bar", selinuxTreeDirectory)
	if err != nil {
...
func TestEnsureWorkspaceFcontextExistingRuleWithNestedConflict(t *testing.T) {
	...
	// ensureTreeFcontext("/data", selinuxTreeDirectory) MUST fail before restorecon.
	var restoreconCalled bool
	mgr := newTestManager(func() (bool, bool, error) { return true, true, nil })
	mgr.runCommand = func(cmd string, args ...string) ([]byte, error) {
		if len(args) > 0 && args[0] == "fcontext" && args[1] == "-l" {
...
func TestEnsureWorkspaceFcontextExistingRuleWithUnrelatedSibling(t *testing.T) {
	...
	// ensureTreeFcontext("/data", selinuxTreeDirectory) => idempotent success.
	mgr := newTestManager(func() (bool, bool, error) { return true, true, nil })
	mgr.runCommand = func(cmd string, args ...string) ([]byte, error) {
		if len(args) > 0 && args[0] == "fcontext" && args[1] == "-l" {
...
func TestRestoreconRecursiveArgv(t *testing.T) {
	...
	if err := mgr.restoreconTree("/data", selinuxTreeDirectory); err != nil {
		t.Fatal(err)
	}
...
func TestCheckWorkspaceRelabelBoundary(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := mgr.checkTreeRelabelBoundary(tc.workspace)
			if tc.wantErr {
				if err == nil {
...
func TestRestoreconRecursiveRefusesNestedMount(t *testing.T) {
	...
	err := mgr.restoreconTree("/opt/ws", selinuxTreeDirectory)
	if err == nil {
...
func TestEnsureWorkspaceFcontextUnsafeFailsClosed(t *testing.T) {
	...
	created, err := mgr.ensureTreeFcontext("/opt/ws", selinuxTreeDirectory)
	if err == nil {
...
func TestSELinuxWorkspaceLockSerializes(t *testing.T) {
	...
	go func() {
		_, _ = mgr.ensureTreeFcontext("/data", selinuxTreeDirectory)
		close(firstDone)
	}()
	...
	go func() {
		_, _ = mgr2.ensureTreeFcontext("/data", selinuxTreeDirectory)
		close(secondDone)
	}()
...
func TestSELinuxWorkspaceLockAcquisitionFailure(t *testing.T) {
	// Lock acquisition failure: ensureTreeFcontext returns error.
...
func TestEquivalenceDestAncestorOfBoundary(t *testing.T) {
	...
	_, err := mgr.ensureTreeFcontext("/parent/data", selinuxTreeDirectory)
	if err == nil {
