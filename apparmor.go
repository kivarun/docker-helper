...
// appArmorParserRunner is the bounded-execution seam of the AppArmor profile
// manager. The context carries the remaining MAC transition budget: the
// runner must terminate the parser (kill and reap the child, leave no
// process behind) and return a bounded actionable error when the budget
// expires, and never report success after a timeout.
type appArmorParserRunner func(ctx context.Context, executable string, args []string) error
...
func newProductionParserRunner() appArmorParserRunner {
	return func(ctx context.Context, executable string, args []string) error {
		...
		if err := configureMACCommandSysProcAttr(cmd); err != nil {
			return fmt.Errorf("cannot prepare bounded %s execution: %w", filepath.Base(executable), err)
		}
		var stderr bytes.Buffer
		cmd.Stderr = &stderr
		if err := cmd.Run(); err != nil {
			if ctxErr := macCommandError(ctx, filepath.Base(executable), err); ctxErr != err {
				return ctxErr
			}
...
func (m *appArmorProfileManager) reloadProfile(ctx context.Context) error {
	return m.runParser(ctx, m.parserPath, []string{"--replace", "--skip-cache", m.mainProfilePath})
}

func (m *appArmorProfileManager) validateProfile(ctx context.Context) error {
	return m.runParser(ctx, m.parserPath, []string{"--skip-kernel-load", "--skip-cache", m.mainProfilePath})
}
...
// rollbackFragment restores the fragment file and re-runs the profile reload
// through the SAME transition context that failed: a rollback never obtains
// an unlimited new lifetime, it consumes the remaining transition budget. A
// rollback that cannot be proven is a failure and the caller fail-closes
// with the fragment restored (the deterministic recovery evidence).
func (m *appArmorProfileManager) rollbackFragment(ctx context.Context, snap *fragmentSnapshot) error {
	if err := m.restoreFragmentFile(snap); err != nil {
		return err
	}
	if err := m.reloadProfile(ctx); err != nil {
		return err
	}
	return nil
}
...
func (m *appArmorProfileManager) addManagedBoundary(ctx context.Context, path string) (boundaryResult, error) {
	...
	if err := m.reloadProfile(ctx); err != nil {
		rollbackErr := m.rollbackFragment(ctx, snap)
		if rollbackErr != nil {
			return boundaryResult{}, fmt.Errorf("reload failed: %w; rollback also failed: %w", err, rollbackErr)
		}
...
func (m *appArmorProfileManager) removeManagedBoundary(ctx context.Context, path string) (boundaryResult, error) {
...
func (m *appArmorProfileManager) check(ctx context.Context) error {
	if err := m.checkPrerequisites(); err != nil {
		return err
	}
	...
	if err := m.validateProfile(ctx); err != nil {
