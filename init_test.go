...
func TestInitExplicitAllowedRoot(t *testing.T) {
	...
	if fc.AllowedRoots[0].Path != allowedRoot {
		t.Errorf("allowed_root = %q, want %q", fc.AllowedRoots[0].Path, allowedRoot)
	}
}
...
func TestResolveAllowedRootTildeExpansion(t *testing.T) {
	...
	if canonicalHome, err := filepath.EvalSymlinks(home); err == nil && validateWorkspacePathSafety(canonicalHome) != nil {
